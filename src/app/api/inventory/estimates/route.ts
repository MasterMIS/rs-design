...
const SHEET_NAME = CONFIG.INVENTORY.ESTIMATES_SHEET;
  ...
  try {
    ...
    const items = data.map((row: string[], index: number) => {
      return {
        ...
        project: row[1] || '',
        itemNo: row[2] || '',
        estimatedQty: row[3] || '0',
      };
    }).filter((t: any) => t.project && t.itemNo);

    return NextResponse.json(items);
  } catch (error: unknown) {
    const err = error as Error;
    console.error('API Error (GET Inventory Estimates):', err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { project, itemNo, estimatedQty } = body;

    if (!project || !itemNo) {
      return NextResponse.json({ error: 'Project and Item No are required.' }, { status: 400 });
    }
    ...
    const err = error as Error;
    console.error('API Error (POST Inventory Estimate):', err);
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const err = error as Error;
    console.error('API Error (PUT Inventory Estimate):', err);
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
